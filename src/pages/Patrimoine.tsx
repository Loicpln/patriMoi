...
// ── Global Recap ───────────────────────────────────────────────────────────────
function GlobalRecap({livrets,positions,ventes,versements,mois}:{livrets:Livret[];positions:Position[];ventes:Vente[];versements:Versement[];mois:string}) {
  const {fmt}=useDevise();
  ...
  const portfolioParPoche=useMemo(()=>{
    ...
    POCHES.forEach(p=>{
      // Merge buys + sells chronologically
  ...
  const evoData=useMemo(()=>evoMonths.map(m=>{
    ...
    // Chronological portfolio aggregation (correct PRU across buy→sell→rebuy)
    type Ev2={date:string;type:"buy"|"sell";ticker:string;qty:number;price:number};
    const evs2:Ev2[]=[
      ...positions.map(p=>({date:p.date_achat??"",type:"buy" as const,ticker:p.ticker,qty:p.quantite,price:p.prix_achat})),
      ...ventes.map(v=>({date:v.date_vente??"",type:"sell" as const,ticker:v.ticker,qty:v.quantite,price:0})),
    ].sort((a,b)=>a.date.localeCompare(b.date));
    const byT:Record<string,{q:number;inv:number}>={};
  ...
  const pieNode=(h:number,_isExp?:boolean)=>inner.length===0?<div className="empty">Aucune donnée</div>:(
    <NestedPie inner={inner} outer={outer} total={grandTotal} fmt={fmt} h={h}
      toggleLabel={pieToggle==="versements"?"→ Valeur":"→ Versements"}
